...
spaceGrid = []
dsep=0.1
spaceCellSize = 1   #basically the resolution
...
def setup():
    ...
    global spaceGrid, spaceCellSize,borderXL,borderXR,borderYT,borderYB
    ###DEFINE SPACE GRID
    spaceRows = int(height/spaceCellSize)
    spaceCols = int(width/spaceCellSize)
...
def checkProximity( xCurr, yCurr):
    ...
    for circ in circleStats:
        if dist (circ[0],circ[1], xCurr,yCurr) < circ[2]:
            return False
    return True
...
def drawCircle(c,r,t, cols):
    ...
    while r<= lastR:
        ...
        while angle<=360:
            ...
            # print(x,y)
            if checkProximity(x,y) and x>borderXL and x<borderXR and y >borderYT and y< borderYB:
                    # print("HI")
                point(x,y)
            angle+=0.1
        r+=1
...
def draw():
    ...
    startingR = int(random(10,30))
    thickness = int(random(10,30))
    gap=int(random(3,15))
    ...
    while flag:
        ...
        if count==1000:
          ...
          noLoop()  
          break
        if checkProximity(centerX,centerY):
            flag=False
    center = [centerX,centerY]
    
    colours = colorChoosing(centerX,centerY)
    ...
    circleStats.append([centerX,centerY,startingR-gap+2])
