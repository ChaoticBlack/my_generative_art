...
class BezierCurve:
    def __init__(self, list_of_points):
        ...
        self.dist_btwn_curves = 8
        # for smallest osculating circle
        self.max_curvature = 0
        self.min_radius = 0
        self.center = PVector(0)

        self.parallel_curves_left = (
            []
        )  # list of list of points of parallel curves to the left
        self.parallel_curves_right = (
            []
        )  # list of list of points of parallel curves to the right
    ...
    def calculate_curve_details(self):
        ...
        t = 0
        while t <= 1:
            bezier_point_vector = self.calculte_curve_points(
                [self.anchor1, self.control1, self.control2, self.anchor2], t
            )
            first_derivative_vector = self.calculate_first_derivative(
                [self.anchor1, self.control1, self.control2, self.anchor2], t
            )
            second_derivative_vector = self.calculate_second_derivative(
                [self.anchor1, self.control1, self.control2, self.anchor2], t
            )
            self.check_inflection_point(second_derivative_vector, bezier_point_vector)
    ...
    def calculte_curve_points(self, list_of_points, t):
        point_x = (
            list_of_points[0][0] * (-1 * t**3 + 3 * t**2 - 3 * t + 1)
            + list_of_points[1][0] * (3 * t**3 - 6 * t**2 + 3 * t)
            + list_of_points[2][0] * (-3 * t**3 + 3 * t**2)
            + list_of_points[3][0] * t**3
        )
        point_y = (
            list_of_points[0][1] * (-1 * t**3 + 3 * t**2 - 3 * t + 1)
            + list_of_points[1][1] * (3 * t**3 - 6 * t**2 + 3 * t)
            + list_of_points[2][1] * (-3 * t**3 + 3 * t**2)
            + list_of_points[3][1] * t**3
        )
    ...
    def calculate_first_derivative(self, list_of_points, t):
        point_x = (
            list_of_points[0][0] * (-3 * t**2 + 6 * t - 3)
        ...
        point_y = (
            list_of_points[0][1] * (-3 * t**2 + 6 * t - 3)
    ...
    def parallel_curves_in_both_directions(self):
        ...
        while dist_to_cover < width + 1000:
            ...
            for index in range(len(self.first_derivative_vector_list)):
                normal_vector = (
                    self.first_derivative_vector_list[index]
                    .copy()
                    .normalize()
                    .rotate(HALF_PI)
                    .mult(dist_to_cover)
                )
                next_curve_right.append(
                    PVector(
                        self.bezier_point_vector_list[index].x + normal_vector.x,
                        self.bezier_point_vector_list[index].y + normal_vector.y,
                    )
                )
                normal_vector = (
                    self.first_derivative_vector_list[index]
                    ...
                    .normalize()
                    .rotate(HALF_PI)
                    .mult(-1 * dist_to_cover)
                )
                next_curve_left.append(
                    PVector(
                        self.bezier_point_vector_list[index].x + normal_vector.x,
                        self.bezier_point_vector_list[index].y + normal_vector.y,
                    )
                )
            self.parallel_curves_right.append(next_curve_right)
            self.parallel_curves_left.append(next_curve_left)
            dist_to_cover = dist_to_cover + self.dist_btwn_curves

    def check_if_tangents_at_all_points(self):
        if len(self.bezier_point_vector_list) != len(self.first_derivative_vector_list):
            print(True)

    def check_inflection_point(self, second_derivative_vector, bezier_point_vector):
    ...
    def calculate_smallest_osculating_circle(self):
        for inflection_point in self.inflection_points_list:
            ...
            if curvature > self.max_curvature:
                ...
                self.center, self.min_radius = self.calculate_radius_and_center(
                    curvature, inflection_point
                )

    def calculate_curvature(self, point_on_curve):
        index_point_on_curve = self.bezier_point_vector_list.index(point_on_curve)
        curvature_direction = PVector.cross(
            self.first_derivative_vector_list[index_point_on_curve],
            self.second_derivative_vector_list[index_point_on_curve],
        )
        curvature_magnitude = (
            self.first_derivative_vector_list[index_point_on_curve].mag() ** 3
        )
    ...
    def calculate_radius_and_center(self, curvature, point_on_curve):
        ...
        center = PVector(
            self.bezier_point_vector_list[index_point_on_curve].x + normal_vector.x,
            self.bezier_point_vector_list[index_point_on_curve].y + normal_vector.y,
        )
    ...
    def draw_osculating_circle_at_inflection(self):
        random_index = int(random(0, len(self.inflection_points_list) - 1))
        (center, radius) = self.calculate_osculating_circle(
            self.inflection_points_list[random_index]
        )
        push()
        noFill()
        stroke(255, 0, 0)
